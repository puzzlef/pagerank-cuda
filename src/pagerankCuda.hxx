...
template <class T>
__global__ void pagerankThreadKernel(T *a, const T *r, const T *c, const int *vfrom, const int *efrom, int N, T c0) {
  DEFINE(t, b, B, G);
...
template <class T>
void pagerankThreadCu(T *a, const T *r, const T *c, const int *vfrom, const int *efrom, int N, T c0, int G, int B) {
  pagerankThreadKernel<<<G, B>>>(a, r, c, vfrom, efrom, N, c0);
}


template <class T>
int pagerankCudaLoop(T *e, T *r0, T *eD, T *r0D, T *&aD, T *&rD, T *cD, const T *fD, const int *vfromD, const int *efromD, const int *vdataD, int N, T p, T E, int L, int G, int B) {
  ...
  for (; l<L; l++) {
    ...
    pagerankThreadCu(aD, rD, cD, vfromD, efromD, N, c0, G, B);
    l1NormCu(eD, rD, aD, N);
...
template <class T>
int pagerankCudaCore(T *e, T *r0, T *eD, T *r0D, T *&aD, T *&rD, T *cD, T *fD, const int *vfromD, const int *efromD, const int *vdataD, int N, T p, T E, int L, int G, int B) {
  fillCu(rD, N, T(1)/N);
  pagerankFactorCu(fD, vdataD, N, p);
  return pagerankCudaLoop(e, r0, eD, r0D, aD, rD, cD, fD, vfromD, efromD, vdataD, N, p, E, L, G, B);
}
...
PagerankResult<T> pagerankCuda(const H& xt, const vector<T> *q=nullptr, PagerankOptions<T> o=PagerankOptions<T>()) {
  ...
  float t = measureDuration([&]() { l = pagerankCudaCore(e, r0, eD, r0D, aD, rD, cD, fD, vfromD, efromD, vdataD, N, p, E, L, G, B); }, o.repeat);
