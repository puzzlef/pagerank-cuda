...
template <class T>
void pagerankThreadCu(T *a, const T *c, const int *vfrom, const int *efrom, int i, int n, T c0, const PagerankOptions<T>& o) {
  int B = o.blockSize;
...
void pagerankSwitchedCu(T *a, const T *c, const int *vfrom, const int *efrom, int i, J&& ns, T c0, const PagerankOptions<T>& o) {
  for (int n : ns) {
    ...
    else     pagerankThreadCu(a, c, vfrom, efrom, i, -n, c0, o);
    i += abs(n);
  }
}
