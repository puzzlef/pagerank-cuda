...
template <class T>
void pagerankScaleSumCu(T *a, const T *x, int N, int SF) {
  switch (SF) {
    default: break;
    case 1:  sumAbsCu(a, x, N); break;
    case 2:  sumSqrCu(a, x, N); break;
  }
}

template <class T>
T pagerankScaleSumReduce(const T *x, int N, int SF) {
  switch (SF) {
    default: return T(1);
...
template <class T, class J>
int pagerankCudaLoop(T *e, T *r0, T *eD, T *r0D, T *&aD, T *&rD, T *cD, const T *fD, const int *vfromD, const int *efromD, const int *vdataD, int i, J&& ns, int N, T p, T E, int L, int EF, int SF) {
  int n = sumAbs(ns);
...
PagerankResult<T> pagerankCuda(const H& xt, const vector<T> *q=nullptr, PagerankOptions<T> o=PagerankOptions<T>()) {
  ...
  float t = measureDurationMarked([&](auto mark) {
    ...
    mark([&] { l = pagerankCudaLoop(e, r0, eD, r0D, aD, rD, cD, fD, vfromD, efromD, vdataD, 0, ns, N, p, E, L, EF, SF); });
  }, o.repeat);
